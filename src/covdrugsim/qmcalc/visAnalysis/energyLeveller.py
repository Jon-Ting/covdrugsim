...
#   Draw their labels
        ...
        for key in self.statesList.keys():
            ...
            self.ax.text(state.leftPointx + state.labelOffset[0], state.leftPointy + state.labelOffset[1] + offset,
                         state.label, color=state.labelColor, verticalalignment='bottom')
            self.ax.text(state.leftPointx + state.textOffset[0], state.leftPointy + state.textOffset[1] - offset,
                         "  " + str(state.energy), color=state.labelColor, verticalalignment='top')
...
#   Draw the dashed lines connecting them
        ...
        self.ax.set_title(r'Energy Profile of $\alpha$-Santonin',loc='center',family='sans-serif',fontsize=12)
        if self.do_legend:
            self.ax.legend()
...
def main(inp_path):
    ...
    # yminE, ymaxE = diagram.DetermineEnergyRange()
    # diagram.Draw(yminE, ymaxE)
    diagram.Draw()
